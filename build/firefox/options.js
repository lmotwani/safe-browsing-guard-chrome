...
async function loadOptions() {
    try {
        const { userOptions } = await browser.storage.sync.get('userOptions');
...
async function saveOptionsToStorage(options) {
    await browser.storage.sync.set({ userOptions: options });
}

async function updateBackgroundScript(options) {
    await browser.runtime.sendMessage({ action: 'updateUserOptions', options });
}
