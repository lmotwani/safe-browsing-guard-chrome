...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('whitelistButton').addEventListener('click', async () => {
        try {
            const { userOptions = { trustedDomains: [] } } = await browser.storage.sync.get('userOptions');
            ...
            await browser.runtime.sendMessage({ action: 'updateUserOptions', options: userOptions });
            isTrusted = !isTrusted;
            updateUI(isTrusted);
        } catch (error) {
...
async function getEducationalResources() {
    try {
        const response = await browser.runtime.sendMessage({ action: 'getEducationalResources' });
