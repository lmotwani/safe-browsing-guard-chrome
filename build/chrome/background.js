...
// Cross-browser compatibility: Use 'browser' namespace (works in Chrome 109+ and Firefox)
// Fallback to 'chrome' for older Chrome versions
...
async function loadUserOptions() {
    try {
        const data = await browser.storage.sync.get(['userOptions']);
...
async function saveUserOptions() {
    try {
        await browser.storage.sync.set({ userOptions });
    } catch (error) {
...
async function hasSiteBeenFlaggedThisWeek(domain) {
    try {
        const data = await browser.storage.local.get([`${domain}_week`]);
    ...
    browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
        const handlers = {
            async continueDownload() {
                try {
                    await browser.downloads.resume(request.downloadId);
            ...
            async cancelDownload() {
                try {
                    const downloadItem = await browser.downloads.search({id: request.downloadId});
                    if (downloadItem.length > 0) {
                        await browser.downloads.cancel(request.downloadId);
                        if (downloadItem[0].filename) {
                            await browser.downloads.removeFile(request.downloadId);
                        }
                        sendResponse({ success: true });
                    } else {
